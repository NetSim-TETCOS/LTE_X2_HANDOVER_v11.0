...
int fn_NetSim_LTE_InitHandover(NETSIM_ID ueId,NETSIM_ID nENBId)
{
	...
	FILE* fp = NULL;
	char snrlog[100];
	sprintf(snrlog, "LTE_UE_SNR_%d.csv", fn_NetSim_GetDeviceIdByConfigId(ueId));
	fp = fopen(snrlog, "a+");
	fprintf(fp, "\n%d,%lf", fn_NetSim_GetDeviceIdByConfigId(ueId), pstruEventDetails->dEventTime);
			...
			for(j=0;j<enbPhy->ca_count;j++)
			{
				...
				if (fp)
				{
					if (j == 0)
					{
						...
						fprintf(fp, ",%lf", info->DLInfo[j].dSNR);
					}
